...
type regexp = {
  nfa : node -> node;
  named_groups : string StringMap.t;
}

let get_names re = StringMap.fold (fun name _ acc -> name :: acc) re.named_groups []

let get_slot name re =
  StringMap.find name re.named_groups

let set_slot name slot re =
  {re with named_groups =
             StringMap.add name slot re.named_groups}
